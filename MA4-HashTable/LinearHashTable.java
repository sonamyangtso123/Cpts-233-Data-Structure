...
class LinearHashTable<K, V> extends HashTableBase<K, V> {
    ...
    public void addElement(K key, V value) {
        ...
        int index = hash;
        //check for empty spot. Increament index untill you find a empty spot
        while (_items.get(index).isEmpty() == false) {
            index++;
            index = (index) % _items.size();

        }

        // add an object in the empty spot
    ...
    public void removeElement(K key) {
        ...
        for (int i = 0; i < _items.size(); i++) {
            //Increament the index untill you match your key 
            if (_items.get(index).getKey() != key) {
                index = (index + 1) % _items.size();

            }
        }
        if (_items.get(index).getKey() == key) {
            // if you your key, delete that spot by marking it is empty
